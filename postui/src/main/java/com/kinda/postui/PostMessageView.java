...
import com.kinda.postui.util.ViewUtil;

/**
 * Post Message view to create post window view
 */
...
public class PostMessageView extends RelativeLayout {
    ...
    public PostMessageView(Context context, AttributeSet attrs) {
        super(context, attrs);
        initialize(attrs, 0);
    }

    public PostMessageView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        initialize(attrs, defStyleAttr);
    }

    private void initialize(AttributeSet attrs, int defStyleAttr) {
        TypedArray a = getContext().obtainStyledAttributes(attrs, R.styleable.PostMessageView, defStyleAttr, 0);

        showArrow = a.getBoolean(R.styleable.PostMessageView_cmv_showArrow, true);
        arrowMargin = a.getDimension(R.styleable.PostMessageView_cmv_arrowMargin, dip2px(5));
        cornerRadius = a.getDimension(R.styleable.PostMessageView_cmv_cornerRadius, 0);
        contentPadding = a.getDimension(R.styleable.PostMessageView_cmv_contentPadding, dip2px(10));
        backgroundColor = a.getColor(R.styleable.PostMessageView_cmv_backgroundColor, 0);
        backgroundColorPressed = a.getColor(R.styleable.PostMessageView_cmv_backgroundColorPressed, 0);

        int intPosition = a.getInt(R.styleable.PostMessageView_cmv_arrowPosition, ArrowPosition.LEFT.getValue());
        arrowPosition = ArrowPosition.getEnum(intPosition);

        int intGravity = a.getInt(R.styleable.PostMessageView_cmv_arrowGravity, ArrowGravity.START.getValue());
...
//                containerParams.addRule(ABOVE, arrowImage.getId());
    ...
    private void updateColors() {
        PostMessageDrawable roundRectDrawable = new PostMessageDrawable(backgroundColor, cornerRadius);
        ...
        Drawable stateDrawable = new PostMessageStateDrawable(Color.TRANSPARENT) {
            @Override
            protected void onIsPressed(boolean isPressed) {
                PostMessageDrawable conRlBackground = (PostMessageDrawable) containerLayout.getBackground();
                if (isPressed) {
